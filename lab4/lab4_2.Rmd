...
-filter only to fish in Alabama
-restrict character
```{r}
...
-run various checks to doublecheck if work is correct
```{r}
x <- filter(fish, between(scalelength, 2, 4))
x
```

-filter, select, and subset do the same thing, but filter and select are better
-FROM NOW, JUST USE FILTER AND SELECT EXCLUSIVELY
...
```
-when you put &, R is going to look for data that meets both conditions
...
filter(fish, lakeid == "AL" | length >= 350)
```
-using | means either or or
-results will include lakes other than AL
...
```
-refining search even more by combining more than one operator


```{r}
filter(fish, scalelength %in% c(2,4))
```
-none bc it's looking for values that are exactly 2 and 4, not between 2 and 4
...
summary(mammals)
```
-values won't be accurate bc many NA's in data
