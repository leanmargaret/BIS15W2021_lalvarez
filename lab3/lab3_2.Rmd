...
if you have spreadsheet in excel, use file.saveas.csv
...
run getwd() in every single r session
```{r}
getwd()
```

if wrong wd, setwd(), then copy and paste file/folder


## Load the data
Here we open a .csv file. Since we are using the tidyverse, we open the file using `read_csv()`. `readr` is included in the tidyverse set of packages. We specify the package and function wit h the `::` symbol. This becomes important if you have multiple packages loaded that contain functions with the same name.  
...
glimpse(hot_springs)
```
-glimpse is another type of way to summarize
-double precision floating numeric data?
...
-change scientist from character to factor by using as.factor
...
difference between factors and character
-factors have specific levels(difference between 1 and 7 vs. 17)
...
-error, so make sure you're directing it to the actual folder it's in, which is the "data" folder
```{r}
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv")
```
...
```
-messy bc continuous variable
-table works best for things that are categorical
