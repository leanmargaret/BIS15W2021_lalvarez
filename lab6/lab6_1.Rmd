...
CHECK IF CODE IS ACCURATE BY USING SUMMARY, SKIM
...
```
IF THERE ARE NA'S THEY WILL SHOW UP HERE
...
```
CAN DO OTHER FUNCTIONS, NOT JUST MEAN
...
```
MEANS WE HAVE MULTIPLE SPECIES WITHIN A SINGLE GENUS
...
msleep %>% 
  summarise(n_genera=n_distinct(genus))
```
77 GENERA BUT 88 ROWS/OBSERVATIONS, SO SOME GENERA ARE REP. BY MORE THAN ONE SPECIES
...
msleep %>%
  ...
  summarize(min_bodywt = min(bodywt),
            ...
            total=n())
```
HOW DO BODY WEIGHTS COMPARE WHEN WE GROUP DIFF KINDS OF VORES TOGETHER
